...
import { getProposalById, roofingProposals } from '@/lib/roofing/proposals';
import { fmtUSD } from '@/lib/dashboard/format';

export const dynamic = 'force-dynamic';

export function generateStaticParams() {
  return roofingProposals.map((p) => ({ id: p.id }));
}
...
export default function RoofingProposalDetailPage({ params }: { params: { id: string } }) {
  const q = getProposalById(params.id);
  ...
  return (
    <>
      <Topbar pageTitle="NoblePort Roofing · Proposal" />
      <RoofingTabs />
      <main className="flex-1 space-y-4 px-4 py-4 sm:px-6 sm:py-6">
        ...
        <section className="panel panel-pad">
          ...
          {q.summary && <p className="mt-3 text-[13px] leading-relaxed text-ink-200">{q.summary}</p>}
        </section>

        {/* Top-line KPIs */}
        <section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          <Stat label="Roof Area (scope)" value={`${q.totalAreaSf.toLocaleString()} SF`} hint={`${q.squares} squares`} />
          <Stat label="Predominant Pitch" value={q.pitch} hint={q.flatAreaSf > 0 ? `${q.flatAreaSf} SF flat` : 'pitched'} />
          <Stat
            label="Pitched / Flat"
            value={`${q.pitchedAreaSf.toLocaleString()}/${q.flatAreaSf.toLocaleString()}`}
            hint="SF · pitched / flat"
          />
          ...
          />
          <Stat label="Duration" value={q.durationDays.split(' ')[0]} hint="weather permitting" />
        </section>
        ...
        <div className="rounded-md border border-yellow-500/30 bg-yellow-500/5 px-4 py-3">
          ...
          <p className="mt-1 text-[13px] leading-relaxed text-ink-200">
            Line-item pricing is computed from field measurements at NoblePort unit rates. This is an{' '}
            <span className="font-semibold">estimate</span>, not a firm bid, until the deck is inspected on site and the
            proposal is signed.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
          ...
          <Panel title="Project Overview" subtitle="measurements">
            <dl className="space-y-1.5 text-sm">
              <Fact label="Roof area (scope)" value={`${q.totalAreaSf.toLocaleString()} SF`} />
              <Fact label="Pitched area" value={`${q.pitchedAreaSf.toLocaleString()} SF`} />
              <Fact label="Flat area" value={`${q.flatAreaSf.toLocaleString()} SF`} />
        ...
        <Panel title="Estimated Investment" subtitle="line-item breakdown" padded={false}>
          ...
          <p className="px-4 py-2.5 text-[11px] leading-relaxed text-ink-400">
            ...
            {fmtUSD(blendedPerSf)}/SF across {q.totalAreaSf.toLocaleString()} SF in scope.
          </p>
        </Panel>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <Panel title="Exclusions" subtitle="not in base price">
            <ul className="space-y-2 text-[13px]">
              {q.exclusions.map((e) => (
                <li key={e} className="rounded-md border border-ink-700 bg-ink-900/40 px-3 py-2 text-ink-300">
                  {e}
                </li>
              ))}
            </ul>
        ...
        <p className="px-1 text-[11px] leading-relaxed text-ink-500">
          ...
          labor and material costs. Final figures subject to on-site deck inspection. Not a firm bid until signed.
        </p>
      </main>
    </>
  );
